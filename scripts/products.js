window.products = [{
    id: 'adidas',
    name: 'классика',
    price: '2 590 ₽',
    category: 'Odezda',
    images: ['img/adidas.png', 'img/adidas.png'],
    description: ['Унисекс; Оверсайз крой', 'Состав: 100% хлопок', 'Шелкотрафаретная печать', 'Плотность: 220 г/м²'],
    care: ['Ручная или машинная стирка до 30°C', 'Глажка с паром с изнанки', 'Избегать горячих поверхностей', 'Сушка в разложенном виде'],
    delivery: ['Доставка оплачивается получателем', 'СДЭК — 2–5 дней', 'Почта России — 5–10 дней'],
    return: 'Обмен и возврат возможен в течение 14 дней с момента получения товара, при сохранении товарного вида и ярлыков.'
}, {
    id: 'kostumadidas',
    name: 'Костюм',
    price: '3 200 ₽',
    category: 'Odezda',
    images: ['img/kostumadidas.png'],
    description: ['Удобный крой', 'Натуральные материалы', "Обычно пульт в пакете в комплекте"],
    care: ['Машинная стирка'],
    delivery: ['Доставка в течение 3 дней'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'shtani',
    name: 'Штаны',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/shtani.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'bomber',
    name: 'Бомбер',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/bomber.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'fufayka',
    name: 'Фуфайка',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/fufayka.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'roba',
    name: 'Роба',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/roba.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'yshanka',
    name: 'Шапка ушанка',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/yshanka.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'platok',
    name: 'Платок на голову',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/platok.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'sviter2',
    name: 'Свитер',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/sviter2.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'sviter-tot-sami',
    name: 'Свиттер',
    price: '490 ₽',
    category: 'Odezda',
    images: ['img/sviter-tot-sami.png'],
    description: ['Мягкий хлопок', 'Яркий принт'],
    care: ['Стирка при 40 градусах'],
    delivery: ['Почта России'],
    return: 'Возврат невозможен для носков.'
}, {
    id: 'kepka koz',
    name: 'Кепка с козырьком',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/kepka koz.jpg'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'kgb',
    name: 'Плащ',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/kgb.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'palto',
    name: 'Пальто',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/palto.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'sapogi',
    name: 'Кирзовые сапоги',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/sapogi.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'tufli-myzh',
    name: 'Туфли',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/tufli-myzh.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'valenki',
    name: 'Валенки',
    price: '5 100 ₽',
    category: 'Odezda',
    images: ['img/valenki.jpg'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'kover',
    name: 'Красный ковер',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/kover.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'paketspaketami',
    name: 'Пакет с пакетами',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/paketspaketami.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'yashik',
    name: 'Ящик',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/yashik.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'pult',
    name: 'Пульт в пакете',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/pult.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'stakan-pust',
    name: 'Граненый стакан наполовину пустой',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/stakan.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'stakan-poln',
    name: 'Граненый стакан наполовину полный',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/stakan.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'hleb',
    name: 'Хлеб',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/hleb.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'voda',
    name: 'Вода в стеклянной бутылке',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/voda.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'spichki',
    name: 'Спички',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/spichki.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'stolkniga',
    name: 'Стол книжка',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/stolkniga.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
}, {
    id: 'lebed',
    name: 'Лебедь из покрышки',
    price: '5 100 ₽',
    category: 'tovari',
    images: ['img/lebed.png'],
    description: ['Защита от ветра', 'Стильный дизайн'],
    care: ['Только сухая чистка'],
    delivery: ['Доставка курьером'],
    return: 'Возврат в течение 14 дней.'
},];